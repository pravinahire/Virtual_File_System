...
void man(char *name)
{
	...
	if(_stricmp(name,"create") == 0)
	{
		...
		printf("Usag: create File_name Permission\n");
	}
	else if(_stricmp(name,"read") == 0)
	{
		printf("Description: Used to read data from regular file\n");
		printf("Usag: read File_name No_Of_Bytes_To_Read\n");
	}
	else if(_stricmp(name,"write") == 0)
	{
		printf("Description: Used to writr data into regular file\n");
		printf("Usag: write File_name \n After this enter the data that we want to write\n");
	}
...
void DisplayHelp()
{
	...
	printf("write:To Write content in file\n");	
...
int GetFDFromName(char *name)
{
	...
	{
		if(_strcmp(UFDArr[i].ptrfiletable->ptrinode->FileName),name) == 0)
	}
	i++;
	}
	if(i == 50)
...
PINODE Get_Inode(char *name)
{
	...
	while(temp!=NULL)
	{
		if(_strcmp(name,temp->FileName) == 0)
		break;
		temp = temp->next;
	}
	return temp;
...
int ReadFile(int fd,char *arr,int size)
{
	...
	{
		strncpy( arr,
			(UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->readoffset),read_size);
	...
	{
		strncpy( arr,
			(UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->readoffset),isize);
...
int WriteFile(int fd,char *arr,int size)
{
	...
	strncpy( arr,
			(UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->writeoffset),arr,isize);
...
int LseekFile(int fd,int size,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->mode == READ) ||(UFDTArr[fd].ptrfiletable->mode = READ+WRITE))
	{
		...
		else if(from == START)
		{
			...
			if(size < 0)
				return -1;
			(UFDTArr[i].ptrfiletable->readoffset) = size;
		}
		else if(from == END)
		{
 	...
 	else if(UFDTArr[fd].ptrfiletable->mode == WRITE)
	{
		if(from == CURRENT)
		{
			if(((UFDTArr[fd].ptrfiletable->writeoffset) + size) > MAXFILSIZE)
				return -1; 
...
int main()
{
	...
	while (1)
	{
		...
		if (count == 1)
		{
			...
			else if (_strcmp(command[0], "halp") == 0)
			{
				DisplayHelp();
				continue;
			}
		...
		else if (count == 3)
		{
			...
			{
				ret = CreateFile(commnad[1], atoi(command[2]));
				if (reg >= 0)
			...
			{
				ret = OpenFile(commnad[1], atoi(command[2]));
				if (reg >= 0)
